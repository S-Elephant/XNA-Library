...
namespace XNALib
{
    ...
    /// http://www.xnawiki.com/index.php?title=Advanced_Sound_Manager_For_XNA_GS_3.1
    /// Use the XactMgr to use XACT files instead.
    /// NOTE: when the sounds are not unloaded/free-ed then make sure you called the Update() methods.
    /// </summary>
    [Obsolete("Use the AudioMgrPooled instead. This one is deprecated but is still included for legacy games.")]
    public class AudioMgr : GameComponent
    {
        #region Members
        public bool EnableMusic = true;
        public bool EnableSound = true;
        public string MusicFolder = "Music/";
        public string SoundFolder = "SoundFX/";
        ...
        public AudioMgr(Game game, string contentFolder)
            : base(game)
        {
        ...
        public void LoadSong(string songName, string songPath)
        {
            ...
            _songs.Add(songName, _content.Load<Song>(MusicFolder + songPath));
        }
        ...
        public void LoadSound(string soundName, string soundPath)
        {
            ...
            _sounds.Add(soundName, _content.Load<SoundEffect>(SoundFolder + soundPath));
        }
        ...
        /// </summary>
        [Obsolete("Doesn't work with mp3. See: http://forums.create.msdn.com/forums/p/53279/322939.aspx#322939")]
        #warning Doesn't work for mp3. See: http://forums.create.msdn.com/forums/p/53279/322939.aspx#322939
...
namespace XNALib
{
    ...
    public class AudioMgr : GameComponent
    {
        ...
        /// </summary>
        /// <param name="songName">Name of the song to load</param>
        public void LoadSong(string songKey)
        {
            LoadSong(Lookup[songKey], Lookup[songKey]);
        }
        ...
        /// <param name="songName">Name of the song to load</param>
        /// <param name="songPath">Path to the song asset file</param>
        public void LoadSong(string songKey, string songPath)
        {
            if (_songs.ContainsKey(Lookup[songKey]))
            {
                throw new InvalidOperationException(string.Format("Song '{0}' has already been loaded", Lookup[songKey]));
            }

            _songs.Add(Lookup[songKey], _content.Load<Song>(MusicFolder+songPath));
        }
        ...
        /// </summary>
        /// <param name="soundName">Name of the sound to load</param>
        public void LoadSound(string soundKey)
        {
            LoadSound(Lookup[soundKey], Lookup[soundKey]);
        }
        ...
        {
            if (_sounds.ContainsKey(Lookup[soundKey]))
            {
                throw new InvalidOperationException(string.Format("Sound '{0}' has already been loaded", Lookup[soundKey]));
            }

            _sounds.Add(Lookup[soundKey], _content.Load<SoundEffect>(SoundFolder+soundPath));
        }
        ...
        /// <param name="songName">Name of the song to play</param>
        /// <param name="loop">True if song should loop, false otherwise</param>
        public void PlaySong(string songKey, bool loop)
        {
            if (EnableMusic)
            {
                if (CurrentSong != Lookup[songKey])
                {
                    if (_currentSong != null)
                    {
                        MediaPlayer.Stop();
                    ...
                    if (!_songs.TryGetValue(Lookup[songKey], out _currentSong))
                    {
                        throw new ArgumentException(string.Format("Song '{0}' not found", Lookup[songKey]));
                    }

                    CurrentSong = Lookup[songKey];
        ...
        {
            if (EnableSound)
                PlaySound(Lookup[soundKey], 1.0f, 0.0f, 0.0f);
        }
        ...
        {
            if (EnableSound)
                PlaySound(Lookup[soundKey], volume, 0.0f, 0.0f);
        }
